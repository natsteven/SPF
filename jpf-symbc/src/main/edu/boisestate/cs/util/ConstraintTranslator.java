	...
	public PrintConstraint translate(DerivedStringExpression dse) {
		...
		switch (op) {
			case CONCAT:
				...
				right.setType(1);
				PrintConstraint concat = new PrintConstraint(translator.getNextID(), dse.left.toString() + ".concat(" + dse.right.toString() + ")", "concat!!Ljava/lang/String;!:!0");
			...
			case SUBSTRING:
				...
				if (dse.oprlist.length == 2) { // substring(i)
					IntegerConstant startIndex = (IntegerConstant) dse.oprlist[1];
					PrintConstraint substring = new PrintConstraint(translator.getNextID(), str.toString() + "substring(" + startIndex.value + ")", "substring!!I!:!0");
					...
					return substring;
				} else if (dse.oprlist.length == 3) { // substring(i,j)
...
//					System.exit(1);
			...
			case DELETE:
				...
				PrintConstraint delete = new PrintConstraint(translator.getNextID(), se3.toString() + ".delete(" + index1.value() + "," + index2.value() + ")", "delete!!II!:!0");
			...
			case INSERT:
				...
				PrintConstraint insert = new PrintConstraint(translator.getNextID(), se5.toString() + ".insert(" + index3.value() + "," + insertStr.toString() + ")", "insert!!ILjava/lang/CharSequence;!:!0");
			...
			case REVERSE:
				...
				rev.reverse();
				PrintConstraint reverse = new PrintConstraint(translator.getNextID(), se6.toString() + ".reverse()", "reverse!!Ljava/lang/String;!:!0");
				...
				return reverse;
			case VALUEOF:
				Expression expr = dse.oprlist[0];
				if (expr instanceof StringSymbolic) {
					return translate((StringSymbolic) expr);
				} else if (expr instanceof StringConstant) {
					return translate((StringConstant) expr);
				} else if (expr instanceof DerivedStringExpression) {
					return translate((DerivedStringExpression) expr);
...
//                System.exit(1);
	...
	public void translate(Constraint numericConstraint) {
		...
		if (left instanceof SymbolicCharAtInteger) {
			...
			leftConstraint.setType(1);
			rightConstraint.setType(0);
		} else if (left instanceof SymbolicLengthInteger) {
...
//				return null;
	...
	public PrintConstraint translate(IntegerExpression ie) {
		if (ie instanceof IntegerConstant) {
			...
			PrintConstraint lengthConstraint = new PrintConstraint(translator.getNextID(), symConstraint.toString(), "length!!!:!0");
			symConstraint.setType(0);
			lengthConstraint.sourceConstraints.add(symConstraint);
			...
			String name = ((SymbolicInteger) ie).getName();
			if (symMap.containsKey(name)) {
				return symMap.get(name);
			}
			int id = translator.getNextID();
			String val = "r" + id + "!:!getStringValue!!";
			PrintConstraint symConstraint = new PrintConstraint(id, name, val);
			symMap.put(name, symConstraint);
			return symConstraint;
